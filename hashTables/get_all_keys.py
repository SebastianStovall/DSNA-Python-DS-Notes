...
class HashTable:
    ...
    def keys(self):
        all_keys = [] # create a list to store all of the hash table's keys
        for bucket in range(len(self.data_map)):  # iterate through each bucket/index in the hash table
            if self.data_map[bucket] is not None: # if there are items in the bucket, loop through them and grab their keys
                for KVP in self.data_map[bucket]:
